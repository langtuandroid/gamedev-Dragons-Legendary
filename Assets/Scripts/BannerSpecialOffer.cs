...
public class BannerSpecialOffer : MonoBehaviour
{
	...
	private void Init()
	{
		...
		textPrice.text = InAppPurchaseManager.GetPrice("matchhero_special_offer", "USD 19.99");
		textJewelCost.text = $"{packageData.jewel:#,###}";
		textTokenCost.text = $"x {packageData.tokenSum}";
	...
	private IEnumerator ProcessUserDataEffect(List<ChestListDbData> _listReward)
	{
		...
		LobbyManager.ShowChestOpen(_listReward, ChestType.Mysterious);
