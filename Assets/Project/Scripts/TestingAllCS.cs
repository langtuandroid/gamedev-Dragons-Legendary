...
public class TestingAllCS : MonoBehaviour
{
	...
	private void cycleThroughExamples()
	{
		...
		{
			UnityEngine.Debug.Log("customTweenExample starting pos:" + ltLogo.transform.position + " origin:" + origin);
		...
		public void updateValueExample()
		{
			...
			Action<float, object> callOnUpdate = updateValueExampleCallback;
			Vector3 eulerAngles = ltLogo.transform.eulerAngles;
			LeanTween.value(gameObject, callOnUpdate, eulerAngles.y, 270f, 1f).setEase(LeanTweenType.easeOutElastic).setOnUpdateParam(hashtable)
				.setUseEstimatedTime(useEstimatedTime);
		}
