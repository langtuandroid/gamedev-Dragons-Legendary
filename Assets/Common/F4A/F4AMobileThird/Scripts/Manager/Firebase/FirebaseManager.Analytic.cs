namespace com.F4A.MobileThird
{
    ...
    public partial class FirebaseManager
    {
        ...
        public void LogEvent(string nameEvent, Dictionary<string, string> values)
        {
            ...
            parameters.Add(new Parameter("country", F4ACoreManager.Instance.ipComponent.country));
            parameters.Add(new Parameter("city", F4ACoreManager.Instance.ipComponent.city));
            parameters.Add(new Parameter("deviceModel", F4ACoreManager.Instance.ipComponent.deviceModel));
            parameters.Add(new Parameter("countryCode", F4ACoreManager.Instance.ipComponent.countryCode));
            parameters.Add(new Parameter("os", Application.platform.ToString()));
            parameters.Add(new Parameter("version", Application.version));
            FirebaseAnalytics.LogEvent(nameEvent, parameters.ToArray());
#endif
            //var events = new Dictionary<string, string>();
            //foreach (var pair in values)
            //{
            //    events[pair.Key] = pair.Value.ToString();
            //}
