...
namespace com.F4A.MobileThird
{
    public partial class AdsManager
    {
                ...
                if (IsEnableAdmobAd())
                {
                    ...
                    for (int i = 0; i < adConfigData.admobConfig.IdDeviceTests.Length; i++)
                    {
                        builder.TagForChildDirectedTreatment(adConfigData.admobConfig.ForChild)
                            .AddTestDevice(adConfigData.admobConfig.IdDeviceTests[i]);
                    }
        ...
        {
#if DEFINE_ADMOB
            return AdSize.IABBanner.Height;
#endif
            return 0;
        }
        ...
        private void Update()
        {
            if (_isShowNative && _unifiedNativeAdLoaded && _unifiedNativeAd != null)
            {
                ...
                if (_registerCallActionObject)
                {
                    ...
                    {
                        Debug.Log("AdsManager.Update RegisterCallToActionGameObject " + _registerCallActionObject.name + " failed");
                    }
                    else
                    {
                        Debug.Log("AdsManager.Update RegisterCallToActionGameObject " + _registerCallActionObject.name + " success");
                    }
                }
            }
    }
        ...
        {
            Debug.Log("AdsManager.AdLoader_OnNativeAdClicked");
        }

        private void AdLoader_OnAdFailedToLoad(object sender, AdFailedToLoadEventArgs e)
        {
            Debug.Log("AdsManager.AdLoader_OnAdFailedToLoad e:" + e.Message);
        }
        ...
        {
            Debug.Log("AdsManager.AdLoader_OnUnifiedNativeAdLoaded");
        ...
        public DMCDataNativeAd GetDataNativeAd()
        {
            ...
            {
                Debug.Log("AdsManager.GetDataNativeAd GetStore:" + _unifiedNativeAd.GetStore());
